...
async def update_indicators_periodically(indicators_dict, target_dict, target_lock):
    ...
    while True:
        ...
        if current_minute != previous_minute:
            ...
            if duration > 30:
                duration_msg = f"⚠️ 보조지표 연산에 소요시간 30초 초과. 총 {duration}초 소요."
                asyncio.create_task(send_webhook(duration_msg))
...
async def update_wallet_realtime(ACCESS_KEY, SECRET_KEY, wallet_dict):
    ...
    while True:
        ...
        async def error_limit_count():
            ...
            if error_limit == 0:
                await send_webhook("⚠️ wallet_dict Rest API 100회 이상 오류 발생. 시스템 종료.")
                sys.exit(0)
        ...
        if "error" in response_data:
            ...
            print(error_msg)
            await send_webhook(error_msg)  # 오류 전송 완료까지 대기
